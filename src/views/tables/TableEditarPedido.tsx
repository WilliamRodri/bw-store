import { Alert, AlertColor, Button, Card, CardActions, CardContent, Checkbox, CircularProgress, Divider, FormControl, Grid, IconButton, InputLabel, MenuItem, Select, Snackbar, Table, TableBody, TableCell, TableHead, TableRow, TextField, Typography } from "@mui/material";
import { DeleteEmpty } from "mdi-material-ui";
import { useRouter } from "next/router";
import { ChangeEvent, useEffect, useState } from "react";

const TableEditarPedido = () => {
    // Status do Alerta
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const responseTwo = await fetch('/api/pedidos');
                if (!responseTwo.ok) {
                    throw new Error('Failed to fetch orders');
                }

                const dataJsonOrders = await responseTwo.json();
                const dataFilter = dataJsonOrders.find((item: any) => item._id === id);
                
                if (dataFilter) {
                    console.log(dataFilter);
                    setModels(data);
                    setModelName(dataFilter.identificacao_pedido);
                    setItemsModelos(dataFilter.modelos);
                    setModelsAdded(true);
                    setLoading(false);
                }
            } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        if (!modelsAdded) {
            ...
            setTextSnackBarAlert('Adicione pelo menos um modelo antes de atualizar o pedido.');
        ...
        try {
            ...
            setTextSnackBarAlert('Pedido atualizar com sucesso!');
            setSnackbarOpen(true);
            
            router.push('/pedido');
        } catch (error) {
            console.error('Error ao atualizar o pedido: ', error);
        }
    }
...
export default TableEditarPedido;