import { Alert, AlertColor, Button, Card, CardActions, CardContent, CircularProgress, Divider, FormControl, Grid, InputLabel, MenuItem, Select, SelectChangeEvent, Snackbar, TextField, Typography } from "@mui/material";
import { useRouter } from "next/router";
import { ChangeEvent, useEffect, useState } from "react";

const TableEditarModelo = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                dataJson.map((item: any) => {
                    data.push({
                        id: item.id,
                        mater_prima_aviamentos: item.mater_prima_aviamentos,
                        un_medida: item.un_medida,
                        preco_unit: item.preco_unit_c_frete,
                    });
                });
                setMaterials(data);

                const responseModelo = await fetch('/api/modelo');
                if (!responseModelo.ok) {
                    throw new Error('Failed to fetch models');
                }

                const dataModelo = await responseModelo.json();
                const modelo = dataModelo.find((item: any) => item._id === id);

                if (modelo) {
                    setValorImposto(modelo.valorImposto);
                    setValorPrecoVenda(modelo.valorPrecoVenda);
                    setModeloData([modelo]);
                    setItems1(modelo.materia_prima);
                    setItems2(modelo.aviamentos);
                    setItems3(modelo.embalagem_outros);
                    setItems4(modelo.mao_de_obra);
                    setModelName(modelo.nome_modelo);
                }
    ...
    const handleConsumption1Change = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedMaterial1) {
            ...
            setTotalValue1(consumption * selectedMaterial1.preco_unit)
        }
    }
    const handleSaveItem1 = () => {
        if (selectedMaterial1 && consumption1 > 0) {
          const newItem = { ...selectedMaterial1, consumo_p_peca: consumption1, preco_total: totalValue1 }
    ...
    const handleConsumption2Change = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedMaterial2) {
          setTotalValue2(consumption * selectedMaterial2.preco_unit)
        }
    }
    const handleSaveItem2 = () => {
        if (selectedMaterial2 && consumption2 > 0) {
          const newItem = { ...selectedMaterial2, consumo_p_peca: consumption2, preco_total: totalValue2 }
    ...
    const handleConsumption3Change = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedMaterial3) {
          setTotalValue3(consumption * selectedMaterial3.preco_unit)
        }
    }
    const handleSaveItem3 = () => {
        
        if (selectedMaterial3 && consumption3 > 0) {
          const newItem = { ...selectedMaterial3, consumo_p_peca: consumption3, preco_total: totalValue3 }
    ...
    const handleConsumption4Change = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedMaterial4) {
          setTotalValue4(consumption * selectedMaterial4.preco_unit)
        }
    }
    const handleSaveItem4 = () => {
        if (selectedMaterial4 && consumption4 > 0) {
          const newItem = { ...selectedMaterial4, consumo_p_peca: consumption4, preco_total: totalValue4 }
    ...
    const handleSubmit = async () => {
        ...
        const formData = {
            ...
            materia_prima: items1.map(item => ({
                materia_prima_aviamentos: item.mater_prima_aviamentos,
                un_medida: item.un_medida,
                price: item.preco_unit,
                consumo_p_peca: item.consumo_p_peca,
                price_total: item.preco_total
            })),
        ...
        try {
                const response = await fetch('/api/update/modelo', {
                    method: 'POST',
                    headers: {
                ...
                setTextSnackBarAlert('Modelo atualizar com sucesso!');
                setSnackbarOpen(true);
                
                router.push('/modelo');
        } catch (error) {
            console.error('Error ao atualizar o modelo: ', error);
        }
    };
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
                    ...
                    <Grid container spacing={5}>
                                ...
                                >
                                    {materials && materials.map((material: any) => (
                                        <MenuItem key={material.id} value={material.id}>
                                            {material.mater_prima_aviamentos}
                                        </MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        ...
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                value={selectedMaterial1 ? selectedMaterial1.un_medida : ''}
                        ...
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                value={selectedMaterial1 ? selectedMaterial1?.preco_unit?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : ''}
                                InputProps={{ readOnly: true }}
                                required={true}
                            />
                        </Grid>
                        ...
                        <div>
                            ...
                            {items1.map((item, index) => (
                                <Grid container spacing={2} key={index} sx={{ marginTop: 2 }}>
                                    ...
                                    <Grid item xs={12} sm={2}>
                                        P.Unit: {item.preco_unit?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                    </Grid>
                                    <Grid item xs={12} sm={2}>
                                        Consumo: {item.consumo_p_peca?.toFixed(2)}
                                    </Grid>
                                    <Grid item xs={12} sm={2}>
                                        Total: {item.preco_total?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                    </Grid>
                    ...
                    <Grid container spacing={5}>
                        ...
                        <TextField
                            ...
                            fullWidth
                            label='Unidade de Medida'
                            value={selectedMaterial2 ? selectedMaterial2.un_medida : ''}
                            InputProps={{ readOnly: true }}
                            required={true}
                        />
                        </Grid>
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                fullWidth
                                label='Preço Unitário'
                                value={selectedMaterial2 ? selectedMaterial2?.preco_unit?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : ''}
                                InputProps={{ readOnly: true }}
                                required={true}
                            />
                        </Grid>
                    ...
                    <Grid container spacing={5}>
                        ...
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                fullWidth
                                label='Unidade de Medida'
                                value={selectedMaterial3 ? selectedMaterial3.un_medida : ''}
                                InputProps={{ readOnly: true }}
                                required={true}
                            />
                        </Grid>
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                fullWidth
                                label='Preço Unitário'
                                value={selectedMaterial3 ? selectedMaterial3?.preco_unit?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : ''}
                                InputProps={{ readOnly: true }}
                                required={true}
                            />
                        </Grid>
                    ...
                    <Grid container spacing={5}>
                        ...
                        <TextField
                            ...
                            fullWidth
                            label='Unidade de Medida'
                            value={selectedMaterial4 ? selectedMaterial4.un_medida : ''}
                            InputProps={{ readOnly: true }}
                            required={true}
                        />
                        </Grid>
                        <Grid item xs={12} sm={2}>
                            <TextField
                                ...
                                fullWidth
                                label='Preço Unitário'
                                value={selectedMaterial4 ? selectedMaterial4?.preco_unit?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : ''}
                                InputProps={{ readOnly: true }}
                                required={true}
                            />
                        </Grid>
...
export default TableEditarModelo;